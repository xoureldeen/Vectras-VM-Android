...
public class SDLSurface extends GLSurfaceView
	...
	public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
		...
		if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.HONEYCOMB) {
			Log.v("Vectras", "Surface Hardware Acceleration: " + this.isHardwareAccelerated());
		}
	...
	public static boolean rightClick(final MotionEvent e) {
	...
	public static boolean leftClick(final MotionEvent e) {
		Thread t = new Thread(new Runnable() {
				public void run() {
					Log.d("SDL", "Mouse Left Click");
					SDLActivity.onNativeTouch(e.getDeviceId(), Config.SDL_MOUSE_LEFT, MotionEvent.ACTION_DOWN, e.getX(),
											  e.getY(), e.getPressure());
					try {
						Thread.sleep(100);
					} catch (InterruptedException ex) {
					...
					SDLActivity.onNativeTouch(e.getDeviceId(), Config.SDL_MOUSE_LEFT, MotionEvent.ACTION_UP, e.getX(),
											  e.getY(), e.getPressure());
				}
			});
		t.start();
	...
	private class GestureListener extends GestureDetector.SimpleOnGestureListener {
		...
		public boolean onSingleTapConfirmed(MotionEvent event) {
			...
			for (int i = 0; i < event.getPointerCount(); i++) {
				...
				// x
				// + "," + y + " P=" + p);
				VectrasSDLActivity.singleClick(event, i);
