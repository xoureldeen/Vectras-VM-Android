		...
		for (int i = 0; i < buttonTexts.length; ++i) {
			...
			if (buttonFlags[i] != 0) {
				...
				if ((buttonFlags[i] & 0x00000002) != 0) {
					mapping.put(111,
							button); /* API 11: KeyEvent.KEYCODE_ESCAPE */
				}
			}
			...
			if (buttonBackgroundColor != Color.TRANSPARENT) {
				...
				if (drawable == null) {
					...
					// setting the color this way keeps the style (gradient,
					// padding, etc.)
...
class SDLMain implements Runnable {
	...
	public void run() {
		...
		// XXX: LIMBO start from vm executor
		// Log.v("SDL", "SDL thread terminated");
		MainActivity.startvm(MainActivity.activity);
		// LIMBO
		...
		for (int i = deviceIds.length - 1; i > -1; i--) {
			...
			if (joystick == null) {
				...
				if ((joystickDevice.getSources() & InputDevice.SOURCE_CLASS_JOYSTICK) != 0
					...
					SDLActivity.nativeAddJoystick(joystick.device_id, joystick.name, 0, -1, joystick.axes.size(),
