package com.epicstudios.vectras.MainRoms;
		...
		myHolder.cdRoms.setOnClickListener(new View.OnClickListener() {
			public void onClick(View view) {
				Thread thread = new Thread(new Runnable() {
					public void run() {
						VMExecutor.extraParams = current.itemExtra;
						try {
							MainActivity.prepareParams(current.itemPath);
						} catch (Exception ex) {
			...
			public void onClick(View v) {
				...
				ad.setButton(Dialog.BUTTON_POSITIVE, "REMOVE "+current.itemName, new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						...
						try {
							Writer output = null;
							File jsonFile = new File(Config.maindirpath + "roms-data" + ".json");
